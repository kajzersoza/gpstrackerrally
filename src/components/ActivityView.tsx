...
import { Coordinate, Split, TrackingStatus, UserSettings } from '../types';
...
interface ActivityViewProps {
  ...
  onLayerChange?: (layer: 'osm' | 'voyager' | 'positron' | 'cyclosm' | 'satellite') => void;
...
export const ActivityView: React.FC<ActivityViewProps> = ({
  ...
  const getActivityIcon = () => {
    switch (settings.activityMode) {
      case 'cycling':
        return <Bike className="w-3.5 h-3.5 text-[#0060e6]" />;
      case 'car':
        return <Car className="w-3.5 h-3.5 text-[#0060e6]" />;
      case 'walking':
      default:
        return <Footprints className="w-3.5 h-3.5 text-[#0060e6]" />;
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-[#f4f7fb]">
      {/* 1. Header (Menu, Title: GPS TRACKER, Settings) */}
      <header className="flex-shrink-0 flex items-center justify-between px-4 py-2.5 bg-white/95 border-b border-slate-100 z-20">
        ...
        <h1 className="text-xl sm:text-2xl font-black tracking-tight text-[#0050cb] select-none font-heading">
          GPS TRACKER
        </h1>
      ...
      {/* Main Content Area (Fixed layout, child splits scroll) */}
      <main className="flex-1 flex flex-col min-h-0 px-3.5 pt-2 pb-2 gap-2.5 overflow-hidden">
        {/* 2. OSM Map Section */}
        ...
        {/* 3. Distance Display Card */}
        <section className="flex-shrink-0 bg-[#eaf2ff] rounded-2xl px-4 py-3.5 sm:py-4 shadow-[0_2px_8px_rgba(0,102,255,0.06)] border border-blue-100/60 flex items-baseline relative">
          <span className="text-[38px] sm:text-[44px] font-black tracking-tight text-[#0060e6] leading-none font-heading">
            {formattedDistanceObj.value}
          </span>
          <span className="text-xl sm:text-2xl font-bold text-[#0060e6] ml-2.5 select-none font-heading">
          ...
          <div className="ml-auto flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-1 bg-white/70 px-2 py-0.5 rounded-full border border-blue-200/50 shadow-2xs">
              {getActivityIcon()}
              <span className="text-[11px] font-bold text-slate-700">{getActivityLabel()}</span>
            </div>

            {trackingStatus === 'running' && (
              <div className="flex items-center gap-1.5 bg-blue-100/90 px-2 py-0.5 rounded-full">
        ...
        {/* 4. Time & GPS Coordinates (2 Columns) */}
        <section className="flex-shrink-0 grid grid-cols-2 gap-2.5">
          {/* Left: Time & Elapsed (Typography matches distance font exactly: Plus Jakarta Sans / font-heading) */}
          <div className="bg-white rounded-2xl px-4 py-3 sm:py-3.5 min-h-[74px] sm:min-h-[82px] shadow-sm border border-slate-100/80 flex flex-col justify-between">
            <span className="text-xs sm:text-[13px] font-bold text-slate-500 tracking-wide">
              {displayClock}
            </span>
            <span className="text-[23px] sm:text-[27px] font-black text-slate-900 tracking-tight leading-tight mt-0.5 font-heading">
              {formatElapsedTime(elapsedSeconds)}
            </span>
          </div>

          {/* Right: Clickable GPS Coordinates Card */}
          <button
            id="btn-coordinates-card"
            onClick={onOpenCoordinates}
            title="Kattints a koordináták részleteihez és megosztásához"
            className="bg-white hover:bg-blue-50/40 active:bg-blue-50/80 rounded-2xl px-4 py-3 sm:py-3.5 min-h-[74px] sm:min-h-[82px] shadow-sm border border-slate-100/80 hover:border-blue-300 flex flex-col justify-center text-left transition-all active:scale-98 cursor-pointer relative group"
          >
            ...
            {settings.coordinateFormat === 'dms' ? (
              <>
                <span className="text-[14px] sm:text-[15.5px] font-bold text-slate-850 font-mono leading-tight tracking-tight pr-4">
                  {dms.latDms}
                </span>
                <span className="text-[14px] sm:text-[15.5px] font-bold text-slate-850 font-mono leading-tight tracking-tight mt-0.5 pr-4">
                  {dms.lngDms}
                </span>
              </>
            ) : (
              <>
                <span className="text-[14px] sm:text-[15.5px] font-bold text-slate-850 font-mono leading-tight pr-4">
                  {activeLat.toFixed(6)}°
                </span>
                <span className="text-[14px] sm:text-[15.5px] font-bold text-slate-850 font-mono leading-tight mt-0.5 pr-4">
                  {activeLng.toFixed(6)}°
                </span>
              </>
            )}
        ...
        {/* 5. Scrollable Splits (Résztávok) List Container */}
        {/* User requirement: "alatta két utolsó rész távolság és részidő , felül az utolsó lefele folymatosan a többi ,,,de nagyon fontos hogy fix képernyő legyen csak a résztávokat lehessen görgetni" */}
            ...
            {trackingStatus !== 'idle' && (
              <div className="bg-[#f0f6ff] rounded-2xl p-2.5 border-2 border-dashed border-[#0060e6]/40 shadow-sm flex items-center justify-between">
                <div className="flex items-center gap-3">
                  {/* Highly visible pulsing split number */}
                  ...
                  <div>
                    <div className="flex items-baseline gap-3 flex-wrap">
                      ...
                      <span className="text-xs sm:text-sm font-bold text-slate-500 font-heading ml-1">
                ...
                {/* Current elapsed time of in-progress split */}
                <div className="text-xs sm:text-sm font-bold text-[#0060e6] font-mono px-2.5 py-1 bg-white rounded-xl shadow-xs border border-blue-100">
                  {Math.floor(currentSplitTimeSec / 60)
                    .toString()
                    .padStart(2, '0')}
                  :
              ...
              splits.map((split) => {
                ...
                return (
                  <div
                    ...
                    className={`rounded-2xl p-3 border-l-4 shadow-[0_2px_6px_rgba(0,102,255,0.04)] flex flex-col gap-1.5 transition-all cursor-pointer ${
                      isFocused
                  ...
                  >
                    {/* Top Row: Index/Name, Distances, Time & Quick Action */}
                    <div className="flex items-center justify-between">
                      {/* Left: Split index / Name & Distances */}
                      <div className="flex items-center gap-2.5 min-w-0">
                        <span
                          className={`text-xl sm:text-2xl font-black tracking-tight w-7 text-center select-none font-heading flex-shrink-0 ${
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="flex items-baseline gap-2 flex-wrap">
                            <span className="text-base sm:text-lg font-black text-slate-900 font-heading leading-tight">
                      ...
                      {/* Right: Time & Edit/Share Action Buttons */}
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        <span className="text-sm sm:text-base font-black text-[#0060e6] font-mono select-none">
                          {split.formattedTime}
                        </span>
                    ...
                    {/* Bottom Row (if notes or photos exist): Preview Badges */}
                    {(hasNotes || hasPhotos) && (
        ...
        {/* 6. Action Buttons (Start, Pause, Resume, Split, Stop) */}
        <section className="flex-shrink-0 grid grid-cols-3 gap-2.5 pt-1">
          ...
          {trackingStatus === 'idle' && (
            <button
              ...
              onClick={onStart}
              className="bg-[#0066ff] hover:bg-[#0054d6] active:bg-[#0048b8] text-white font-black text-base sm:text-lg py-3 rounded-2xl shadow-[0_4px_12px_rgba(0,102,255,0.3)] active:scale-95 transition-all flex items-center justify-center gap-1.5 cursor-pointer font-heading"
            >
          ...
          {trackingStatus === 'running' && (
            <button
              ...
              onClick={onPause}
              className="bg-[#e67e00] hover:bg-[#c96e00] active:bg-[#a85c00] text-white font-black text-base sm:text-lg py-3 rounded-2xl shadow-[0_4px_12px_rgba(230,126,0,0.3)] active:scale-95 transition-all flex items-center justify-center gap-1.5 cursor-pointer font-heading"
            >
          ...
          {/* Split / Résztáv Button */}
          <button
            id="btn-split-lap"
            onClick={onSplit}
            disabled={trackingStatus === 'idle'}
            className={`font-black text-base sm:text-lg py-3 rounded-2xl transition-all flex items-center justify-center gap-1.5 font-heading ${
              trackingStatus === 'idle'
